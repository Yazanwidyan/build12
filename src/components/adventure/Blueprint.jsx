...
const SECTIONS = [
  {
    ...
    label: "Header",
    icon: "🔝",
    hint: "Site title + navigation",
  },
  {
    key: "hero",
    label: "Hero Section",
    icon: "⭐",
    hint: "Big headline + button",
  },
        ...
        return (
          ...
          >
            ...
            <div className="flex-1">
              <p
                className={`text-sm font-semibold ${isBuilt ? "text-teki-700" : "text-gray-500"}`}
              >
                {s.label}
              </p>
              <p className="text-sm text-gray-400">{s.hint}</p>
            </div>
            <span className="text-base">{isBuilt ? "✅" : "⬜"}</span>sasa
          </motion.div>
        );
      })}
