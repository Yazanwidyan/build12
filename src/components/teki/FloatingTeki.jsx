...
const IFRAME_TOP = 54; // browser chrome height within the preview panel (same as AdventureOverlay)
const DEFAULT_TOP = 200; // fallback vertical position within the Teki panel
...
export default function FloatingTeki() {
  ...
  const travelTop = useMemo(() => {
    ...
    return Math.max(8, getSectionCenter(highlightSection, sections) - 36);
  }, [highlightSection, headerBuilt, heroBuilt, footerBuilt]);

  const effectiveTop = travelTop ?? DEFAULT_TOP;

  // If this step doesn't own a section highlight, clear any leftover one from the previous step
  useEffect(() => {
    const hasHighlight =
  ...
  return (
    <motion.div
      className="absolute z-10 flex items-start gap-2 select-none "
      style={{ right: 12, left: 12 }}
      ...
      <div className="flex-1 flex flex-col gap-2 min-w-0">
        ...
        {shownText && (
          <div className="relative pointer-events-auto">
            {/* Border tail */}
            <div
              className="absolute -right-[13px] top-1/2 -translate-y-1/2 w-0 h-0"
              style={{
            ...
            {/* Fill tail */}
            <div
              className="absolute -right-[11px] top-1/2 -translate-y-1/2 w-0 h-0"
              style={{
            ...
            />
            <div
              className="rounded-2xl px-4 py-3 text-sm leading-relaxed font-mono"
              style={{
        ...
        {/* Step interaction — always mounted so speak() fires exactly once per step.
            Hidden while Teki is typing, revealed when she's done. */}
        <div
