...
const STARS = [
  { x:5,  y:8,  s:2 }, { x:14, y:3,  s:1 }, { x:24, y:11, s:3 }, { x:38, y:5,  s:1 }, { x:50, y:9,  s:2 },
  { x:62, y:4,  s:1 }, { x:74, y:12, s:2 }, { x:85, y:6,  s:3 }, { x:94, y:10, s:1 }, { x:8,  y:22, s:1 },
  { x:20, y:28, s:2 }, { x:33, y:20, s:1 }, { x:45, y:25, s:3 }, { x:58, y:18, s:1 }, { x:70, y:26, s:2 },
  { x:80, y:20, s:1 }, { x:92, y:28, s:2 }, { x:3,  y:40, s:2 }, { x:16, y:48, s:1 }, { x:28, y:38, s:3 },
  { x:40, y:44, s:1 }, { x:55, y:42, s:2 }, { x:66, y:50, s:1 }, { x:78, y:38, s:2 }, { x:90, y:45, s:1 },
  { x:7,  y:60, s:1 }, { x:18, y:65, s:2 }, { x:32, y:58, s:1 }, { x:48, y:68, s:3 }, { x:60, y:62, s:1 },
  { x:72, y:70, s:2 }, { x:84, y:60, s:1 }, { x:96, y:65, s:2 }, { x:10, y:78, s:2 }, { x:25, y:82, s:1 },
  { x:42, y:76, s:2 }, { x:56, y:84, s:1 }, { x:68, y:78, s:3 }, { x:82, y:82, s:1 }, { x:93, y:75, s:2 },
]

// Constellation line pairs (index into STARS)
const LINES = [
  [0,1],[1,2],[2,4],[4,6],[6,7],[8,9],[9,11],[11,13],[13,15],[16,17],
  [17,19],[19,21],[21,23],[23,25],[25,26],[26,28],[28,30],[30,32],[32,34],
  [34,36],[36,38],[38,39],
]
...
function HeroDark() {
  return (
    <div className="absolute inset-0 overflow-hidden">
      {/* Deep space base */}
      <div className="absolute inset-0" style={{ background:"linear-gradient(160deg,#03060e 0%,#050c1a 30%,#080f20 55%,#050918 80%,#03060e 100%)" }} />

      {/* Atmosphere glow — centre */}
      <div className="absolute inset-0 pointer-events-none" style={{ background:"radial-gradient(ellipse 70% 60% at 50% 40%,rgba(44,186,255,0.08) 0%,rgba(44,186,255,0.03) 45%,transparent 70%)" }} />
      ...
      >
        <div style={{ width:"100%", height:"100%", borderRadius:"50%", background:"radial-gradient(circle at 35% 35%,rgba(80,100,200,0.18),rgba(40,60,160,0.08) 50%,transparent 70%)", border:"1px solid rgba(44,186,255,0.08)" }} />
      </motion.div>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity:0.18 }}>
        {LINES.map(([a,b],i) => (
          <line key={i}
            ...
            stroke="rgba(44,186,255,0.6)" strokeWidth="0.5"
          />
        ))}
      </svg>
      ...
      <motion.div className="absolute left-0 right-0 pointer-events-none"
        style={{ height:1, background:"linear-gradient(90deg,transparent,rgba(44,186,255,0.2) 40%,rgba(44,186,255,0.2) 60%,transparent)" }}
        animate={{ top:["0%","100%"] }}
...
function HeroLight() {
  return (
    <div className="absolute inset-0 overflow-hidden">
      {/* Soft sky base */}
      <div className="absolute inset-0" style={{ background:"linear-gradient(160deg,#eef4ff 0%,#f2f0ff 30%,#eaf6ff 55%,#f0f4ff 80%,#eef4ff 100%)" }} />

      {/* Centre atmosphere */}
      <div className="absolute inset-0 pointer-events-none" style={{ background:"radial-gradient(ellipse 70% 60% at 50% 40%,rgba(44,186,255,0.1) 0%,rgba(44,186,255,0.04) 45%,transparent 70%)" }} />

      {/* Top-right planet */}
      <div className="absolute pointer-events-none"
        style={{ top:"-12%", right:"-8%", width:320, height:320, borderRadius:"50%", background:"radial-gradient(circle at 35% 35%,rgba(44,186,255,0.14),rgba(100,140,255,0.06) 50%,transparent 70%)", border:"1px solid rgba(44,186,255,0.12)" }} />
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity:0.22 }}>
        {LINES.map(([a,b],i) => (
          <line key={i}
            x1={`${STARS[a].x}%`} y1={`${STARS[a].y}%`}
            x2={`${STARS[b].x}%`} y2={`${STARS[b].y}%`}
            stroke="rgba(44,186,255,0.7)" strokeWidth="0.5"
          />
        ))}
      </svg>

      {/* Star dots — coloured for light */}
      {STARS.map((s,i) => (
        <motion.div key={i} className="absolute rounded-full pointer-events-none"
          style={{ left:`${s.x}%`, top:`${s.y}%`, width:s.s+0.5, height:s.s+0.5, backgroundColor:"#2cbaff" }}
          animate={{ opacity:[0.35,0.1,0.35], scale:[1,1.3,1] }}
          transition={{ duration:2+(i%5)*0.6, repeat:Infinity, ease:"easeInOut", delay:i*0.08 }} />
      ))}
...
export default function LandingPage() {
  ...
  const dark = !light;
  const txt  = dark ? "#f1f5f9"               : "var(--ink)";
  const sub  = dark ? "rgba(255,255,255,0.5)"  : "var(--ink-muted)";
  const dim  = dark ? "rgba(255,255,255,0.35)" : "var(--ink-faint)";
  const div  = dark ? "1px solid rgba(44,186,255,0.15)" : "2px solid var(--app-border)";
  ...
  return (
    <div className="min-h-screen bg-app flex flex-col">
      ...
      {/* ── Hero ── */}
      <section className="relative overflow-hidden flex items-center justify-center" style={{ minHeight:"92vh" }}>

        {light ? <HeroLight /> : <HeroDark />}

        {/* Dark / Light toggle */}
        <div className="absolute top-4 right-6 z-20 flex items-center gap-1 p-1 rounded-xl"
          style={{ background: dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)", border: dark ? "1px solid rgba(255,255,255,0.12)" : "1px solid rgba(0,0,0,0.09)" }}>
          {[["Dark",false],["Light",true]].map(([label,val]) => (
            <button key={label} onClick={() => setLight(val)}
              className="px-3 py-1 rounded-lg text-xs font-semibold transition-all"
              style={{
                background: light===val ? "rgba(44,186,255,0.22)" : "transparent",
                color: light===val ? "#2cbaff" : dark ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.35)",
                border: light===val ? "1px solid rgba(44,186,255,0.4)" : "1px solid transparent",
              }}>
              {label}
            </button>
          ))}
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 w-full flex flex-col lg:flex-row items-center gap-12 py-20">
          ...
          <div className="flex-1 flex flex-col gap-7 items-start text-left">
            ...
            <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}
              ...
              style={{ background:"rgba(44,186,255,0.12)", border:"2px solid rgba(44,186,255,0.28)", color: dark ? "#7cd8ff" : "#0a90d4" }}>
              <Zap size={12} fill={dark ? "#7cd8ff" : "#0a90d4"} />
              Learn · Build · Launch
            </motion.div>
            ...
            <motion.p
              ...
              style={{ color: sub }}>
              Build real websites, games, and apps — step by step, guided by TEKI your AI companion. No experience needed.
            </motion.p>
            ...
            <motion.div
              ...
              {BUILD_TAGS.map(t => (
                <span key={t.label}
                  ...
                  style={{ background: dark ? "rgba(255,255,255,0.06)" : "rgba(44,186,255,0.08)", border: dark ? "1px solid rgba(255,255,255,0.1)" : "1px solid rgba(44,186,255,0.2)", color: dark ? "rgba(255,255,255,0.65)" : "#0a90d4" }}>
                  {t.emoji} {t.label}
                </span>
              ))}
            </motion.div>
            ...
            <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4, delay:0.28 }}>
              <Button variant="solid" color="blue" size="xl" iconRight={<ArrowRight size={17} />} onClick={handleStart}>
            ...
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.36 }}
              ...
              {[
                ...
                <div key={s.label} className="flex flex-col">
                  <span className="text-2xl font-black" style={{ color: txt }}>{s.value}</span>
                  <span className="text-sm" style={{ color: dim }}>{s.label}</span>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 w-full">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {FEATURES.map((f,i) => (
            <motion.div key={f.label} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1+i*0.07 }}
