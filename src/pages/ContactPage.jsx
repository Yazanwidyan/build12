...
const CONTACT_OPTIONS = [
  { icon: <Mail size={20} />,        title: "Email us",          desc: "hello@helloworldkids.org",   color: "#2cbaff"  },
    ...
    <div className="min-h-screen bg-app flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-6 py-16 w-full flex flex-col gap-16">
        ...
        >
          ...
          <p className="text-xl text-muted max-w-md mx-auto">We'd love to hear from you — whether it's a question, a bug, or just a hello.</p>
        </motion.section>
            ...
            {CONTACT_OPTIONS.map((opt, i) => (
              ...
              >
                <div
                  ...
                  style={{ background: opt.color + "18", color: opt.color, border: `2px solid ${opt.color}35` }}
                >
                  {opt.icon}
                </div>
                <div>
          ...
          >
            {sent ? (
              <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
                ...
                <p className="text-lg text-muted max-w-sm">Thanks for reaching out. We'll get back to you within 24 hours.</p>
                <Button variant="outline" color="neutral" onClick={() => setSent(false)}>Send another</Button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Input label="Your name" placeholder="Ada Lovelace" value={form.name} onChange={set("name")} />
                  <Input label="Email address" type="email" placeholder="ada@example.com" value={form.email} onChange={set("email")} />
                </div>

                <Input label="Subject" placeholder="What's it about?" value={form.subject} onChange={set("subject")} />

                <div className="flex flex-col gap-1.5">
                  <label className="text-base font-semibold" style={{ color: "var(--ink)" }}>Message</label>
                  <textarea
                    rows={5}
